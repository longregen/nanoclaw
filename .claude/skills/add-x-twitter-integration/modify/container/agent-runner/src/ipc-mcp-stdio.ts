...
/**
 * Wait for an X integration result from the host.
 * The host writes result files to the x_results IPC directory.
 */
...
// These tools write IPC task files with x_* types. The host-side handler
// (add-x-twitter-integration/host.ts) processes them and writes results
// back to x_results/ for polling.
